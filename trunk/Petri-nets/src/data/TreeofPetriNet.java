...
public class TreeofPetriNet {
	 final static int Size = 8000;
	  ...
	  public TreeofPetriNet(int np,int nt,int[][] di, int[][] dq, int[] m0,int[] typecrossing){
	    Nt = nt;
	    Np = np;
	    DI = new int[Nt][Np];
	    save2DArray(DI,di);
	  ...
	  //функци проверки существования разрешенных переходов.
	  //Переход tj называется разрешенным в маркеровке М, если М>=I(tj)
	  ...
	  //процедура срабатывания перехода. срабатывание изменяет маркировку сети
	  ...
	  private static void WriteMarker(int Level, int Num, int [] M, int Num2, int[] P, String Str){
	    int i;
	    String S = "";
	    ...
	    //MyWindow.tbTree.setValueAt("M"+ Integer.toString(Num2)+ "("+getStringValue(P,Np)+")", Number-1,2);
	    System.out.print("M"+ Integer.toString(Num2)+ "("+getStringValue(P,Np)+")  ");
	    if (RepeatCount != 0){
	      S = ""; //getStringValue(LastMCrossing,Nt);
	      if (LastCross != 0) S = S + " t" + Integer.toString(LastCross);
	      for (i = 0; i < Nt; i++)
	        if (LastMCrossing[i] == 1) S = S + " m" + Integer.toString(i);
	     // MyWindow.tbTree.setValueAt(S, Number - 1, 3);
	      System.out.print(S+ " ");
	    }
	    //MyWindow.tbTree.setValueAt("M"+ Integer.toString(Num)+ "("+getStringValue(M,Np)+")",Number-1,4);
	    System.out.print("M"+ Integer.toString(Num)+ "("+getStringValue(M,Np)+")  ");
	    ...
	    if (RepeatCount != 0){
	      ...
	      if (Num < Num2) Z[RepeatCount - 1].addNapryam(-1);
	      if (Num >= Num2) Z[RepeatCount - 1].addNapryam(1);
	    }
	  }
	 ...
	 public static void save2DArray(int[][]A, int[][] B){
	  ...
	  private static void Next(){
	    ...
	    if (canedit) {Level++; canedit =false; };
